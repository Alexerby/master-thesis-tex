...
A key reason for beta errors is that benefit eligibility often depends on precise income thresholds. Even small errors or missing values in reported income or household information can shift individuals across eligibility thresholds \citep{herber_non-take-up_2019}. In some cases, relevant parental information was not available in the data, which limited the ability to simulate eligibility accurately. Additional challenges include gaps in information on assets, as well as difficulties capturing criteria such as education type or study progress, which can also affect BAföG entitlement.

To address these limitations, several strategies are used in the literature. These include conducting sensitivity checks by adjusting income levels and applying post-simulation corrections to reclassify borderline cases \citep{herber_non-take-up_2019}. Some beta errors are likely unavoidable due to missing or incomplete information in the available data, such as cases where key parental records are not part of the SOEP sample. In these situations, we rely on internal consistency checks and sensitivity analyses to assess the robustness of eligibility estimates. While it's not possible to resolve all such gaps, recognising their presence helps to interpret the non-take-up results with appropriate caution.

\textcolor{red}{MAYBE ELABORATE FURTHER ON WHAT WE THEN ACTUALLY DO ABOUT THESE ERRORS}
